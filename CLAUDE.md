...
`benadis-runner` — это инструмент автоматизации для систем 1C:Enterprise, написанный на Go. Он оркестрирует сложные рабочие процессы, включая конвертацию данных, операции с базами данных, управление конфигурациями и интеграцию с внешними системами, такими как EDT, SonarQube и Gitea.

## Обзор архитектуры

### Поток обработки запроса
1. **Точка входа** (`cmd/benadis-runner/main.go`): Получает команду через переменную окружения `BR_COMMAND`
...
- `cmd/benadis-runner/main.go` - Маршрутизатор команд (switch-выражение для всех команд)
