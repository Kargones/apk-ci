...
Этот документ описывает тест `main_test.go`, который эмулирует запуск приложения benadis-runner из Gitea Action с параметрами из `action.yaml`.
...
| Параметр action.yaml | Переменная окружения | Переменная benadis-runner |
|---------------------|---------------------|---------------------------|
...
```bash
# Запуск всех тестов в директории cmd/benadis-runner
