# Epic 0: Extension Publish

**Статус:** ✅ Завершён (Legacy-архитектура)
**Приоритет:** ★ ЗАВЕРШЁН
**Риск:** 🟢 Низкий (реализован)
**Stories:** 8/8 (100%)
**FRs:** NEW (вне основной нумерации FR1-68)
**Дата завершения:** 2025-12-08
**Аудит:** 2026-01-26

---

## 📊 Gap Analysis (Аудит 2026-01-26)

### Статус реализации: ✅ ЗАВЕРШЁН

| Компонент | План | Реализация | Статус |
|-----------|------|------------|--------|
| GetLatestRelease() | Story 0.1 | `internal/entity/gitea/gitea.go` | ✅ |
| SearchAllRepos() | Story 0.2 | `internal/entity/gitea/gitea.go` | ✅ |
| GetProjectSubscriptions() | Story 0.3 | `internal/app/extension_publish.go:39` | ✅ |
| SyncExtensionDirectory() | Story 0.4 | `internal/app/extension_publish.go` | ✅ |
| CreatePR with release info | Story 0.5 | `internal/entity/gitea/gitea.go` | ✅ |
| Команда extension-publish | Story 0.6 | `cmd/apk-ci/main.go:244` | ✅ |
| Error handling & reporting | Story 0.7 | `internal/app/extension_publish.go` | ✅ |
| Unit & Integration тесты | Story 0.8 | `extension_publish_test.go` (92.9%) | ✅ |

### ⚠️ Архитектурный GAP

**Проблема:** Реализовано в **legacy-стиле** (switch-case в main.go), а не через Command Registry.

```go
// Текущая реализация (legacy):
case constants.ActExtensionPublish:
    err = app.ExtensionPublish(&ctx, l, cfg)

// Ожидаемая архитектура (Epic 1):
handler := command.Get("extension-publish")
err = handler.Execute(ctx, cfg)
```

### TODO для полной архитектурной совместимости

- [ ] Миграция на Command Registry после Epic 1
- [ ] Добавить как Story 1.10 в Epic 1
- [ ] Пометить legacy case как @deprecated

### Метрики качества

| Метрика | Значение | Требование |
|---------|----------|------------|
| Test Coverage | 92.9% | 80% ✅ |
| Code Review Issues | 0 open | 0 ✅ |
| Ретроспектива | Проведена | ✅ |

---

## Цель

Реализовать команду `extension-publish`, которая при создании релиза в репозитории расширения автоматически распространяет обновлённую версию во все "подписанные" репозитории через механизм веток-подписок.

## Ценность

Автоматизация распространения расширений 1C между репозиториями без ручного копирования.

## Бизнес-контекст

- Репозиторий расширения является источником истины
- При создании релиза автоматически обновляются все "подписанные" репозитории
- Подписка осуществляется через файл `project.yaml` в корне целевого репозитория, секция `subscriptions`
- Формат подписки: `{Org}_{Repo}_{ExtDir}` (например: `lib_ssl_апкБСП`)

## Триггер запуска

- **Event:** `on: release` (Gitea Actions)
- **Режим:** Только автоматический

---

## Stories

### Story 0.1: Расширение Gitea API для релизов

**Приоритет:** P0 | **Размер:** S | **Риск:** Low

**As a** система,
**I want** получать информацию о релизах через Gitea API,
**So that** я могу определить версию для публикации.

**Acceptance Criteria:**

- [ ] Метод `GetLatestRelease()` возвращает последний релиз
- [ ] Метод `GetReleaseByTag()` возвращает релиз по тегу
- [ ] Структура `Release` содержит: tag_name, name, body, assets

**Technical Notes:**
- Файл: `internal/entity/gitea/gitea.go`
- Gitea API: `GET /repos/{owner}/{repo}/releases/latest`

---

### Story 0.2: Расширение Gitea API для поиска репозиториев

**Приоритет:** P0 | **Размер:** S | **Риск:** Low

**As a** система,
**I want** искать репозитории по всей организации,
**So that** я могу найти все потенциальные подписчики.

**Acceptance Criteria:**

- [ ] Метод `SearchAllRepos()` возвращает все репозитории с пагинацией
- [ ] Поддержка фильтрации по организации
- [ ] Обработка пагинации (limit/page)

**Technical Notes:**
- Файл: `internal/entity/gitea/gitea.go`
- Gitea API: `GET /repos/search`

---

### Story 0.3: Поиск подписанных репозиториев

**Приоритет:** P0 | **Размер:** M | **Риск:** Medium

**As a** система,
**I want** найти все репозитории, подписанные на расширение,
**So that** я знаю куда публиковать обновление.

**Acceptance Criteria:**

- [ ] Чтение файла `project.yaml` из корня каждого репозитория
- [ ] Парсинг секции `subscriptions` (массив строк)
- [ ] Формат подписки: `{Org}_{Repo}_{ExtDir}` (например: `lib_ssl_апкБСП`)
- [ ] Если `project.yaml` не существует — репозиторий не подписан
- [ ] Если секция `subscriptions` отсутствует или пуста — репозиторий не подписан
- [ ] Возврат списка: repo + target branch + target directory

**Technical Notes:**
- Файл: `internal/app/extension_publish.go`
- Новая функция: `GetProjectSubscriptions(api, branch) ([]string, error)`
- Удалить: `ParseSubscriptionBranch()`, `IsSubscriptionBranch()`, `excludedBranches`
- Пример `project.yaml`:
  ```yaml
  subscriptions:
    - lib_ssl_апкБСП
    - lib_common_cfe_utils
  ```

---

### Story 0.4: Синхронизация каталога расширения

**Приоритет:** P0 | **Размер:** M | **Риск:** Medium

**As a** система,
**I want** синхронизировать файлы расширения в целевой репозиторий,
**So that** подписчик получает актуальную версию.

**Acceptance Criteria:**

- [ ] Полная замена содержимого целевого каталога
- [ ] Commit message содержит версию и источник
- [ ] Формат: `chore(ext): update {extName} to {version}`

**Technical Notes:**
- Файл: `internal/app/extension_publish.go`
- Использует `SetRepositoryState()` для batch-операций

---

### Story 0.5: Создание PR с полной информацией

**Приоритет:** P0 | **Размер:** S | **Риск:** Low

**As a** система,
**I want** создавать PR с информацией о релизе,
**So that** maintainer понимает что изменилось.

**Acceptance Criteria:**

- [ ] Title: `Update {extName} to {version}`
- [ ] Body содержит: версию, release notes, ссылку на релиз
- [ ] Автоматический assignee (если настроен)

**Technical Notes:**
- Файл: `internal/entity/gitea/gitea.go`
- Расширение существующего `CreatePR()`

---

### Story 0.6: Интеграция команды extension-publish

**Приоритет:** P0 | **Размер:** S | **Риск:** Low

**As a** DevOps-инженер,
**I want** запускать команду через BR_COMMAND,
**So that** публикация интегрирована в CI/CD.

**Acceptance Criteria:**

- [ ] Константа `CmdExtensionPublish` в constants.go
- [ ] Функция `ExtensionPublish()` в app/
- [ ] Case в main.go switch

**Technical Notes:**
- Файлы: `internal/constants/constants.go`, `cmd/apk-ci/main.go`

---

### Story 0.7: Обработка ошибок и отчётность

**Приоритет:** P0 | **Размер:** M | **Риск:** Medium

**As a** DevOps-инженер,
**I want** получать детальный отчёт о результатах публикации,
**So that** я знаю что удалось и что нет.

**Acceptance Criteria:**

- [ ] Continue on error — одна ошибка не блокирует остальные
- [ ] Итоговый отчёт: успешные / неуспешные / пропущенные
- [ ] Exit code = 1 если хотя бы одна ошибка

**Technical Notes:**
- Паттерн: collect errors, report at end
- JSON output для интеграции

---

### Story 0.8: Unit и Integration тесты

**Приоритет:** P1 | **Размер:** M | **Риск:** Low

**As a** разработчик,
**I want** иметь тесты для логики публикации,
**So that** регрессии обнаруживаются автоматически.

**Acceptance Criteria:**

- [ ] Покрытие >80% для extension_publish.go
- [ ] Mock Gitea API для unit-тестов
- [ ] Integration test с реальным Gitea (опционально)

**Technical Notes:**
- Файл: `internal/app/extension_publish_test.go`
- Использовать httptest для mock

---

## Архитектурные решения

| Файл | Содержимое |
|------|------------|
| `internal/constants/constants.go` | Константа `CmdExtensionPublish` |
| `internal/entity/gitea/gitea.go` | Новые методы API |
| `internal/entity/gitea/types.go` | Структура `Release`, расширение `Repository`, структура `ProjectYAML` |
| `internal/app/extension_publish.go` | Основная логика команды |
| `cmd/apk-ci/main.go` | Точка входа (case в switch) |

### Механизм подписок

**Подход:** Декларативная подписка через `project.yaml` в целевом репозитории.

**Структура:**
```yaml
# project.yaml в корне целевого репозитория
subscriptions:
  - lib_ssl_апкБСП        # {Org}_{Repo}_{ExtDir}
  - lib_common_cfe_utils
```

**Алгоритм поиска подписчиков:**
1. Сканируем все организации
2. Для каждого репозитория читаем `project.yaml` (ветка main)
3. Парсим секцию `subscriptions`
4. Если `{sourceOrg}_{sourceRepo}_{extDir}` найден в списке → репозиторий подписан

**Удаляемый код (устаревший механизм веток-подписок):**
- `ParseSubscriptionBranch()`
- `IsSubscriptionBranch()`
- `excludedBranches`

## Зависимости

### Зависимости от существующего кода

- `Config.AnalyzeProject()` — получение имени проекта и расширения
- `GetFileContent()` — чтение `project.yaml` из репозитория
- `SetRepositoryState()` — batch-операции с файлами
- `CreatePR()` — создание Pull Request

### Внешние зависимости

- Gitea API v1
- Gitea Actions runtime

---

## Risk Assessment

| Риск | Вероятность | Импакт | Митигация |
|------|-------------|--------|-----------|
| Массовое изменение репозиториев | Средняя | Высокий | Dry-run режим, детальное логирование |
| API rate limits Gitea | Низкая | Средний | Пагинация, задержки между запросами |
| Ошибка в одном репо блокирует остальные | Средняя | Высокий | Continue on error, отчёт в конце |

---

## Definition of Done

- [ ] Все Acceptance Criteria выполнены
- [ ] Code review пройден
- [ ] Unit-тесты написаны и проходят (>80% coverage)
- [ ] Команда работает в Gitea Actions workflow
- [ ] Протестировано на тестовом окружении
- [ ] Документация обновлена

---

## Связанные документы

- [Epic Overview](./index.md)
- [Детальная документация](../../docs/epics/extension-publish.md)

---

_Последнее обновление: 2026-01-26_
_Аудит проведён: 2026-01-26 (BMAD Party Mode)_
