...
Это руководство поможет диагностировать и решить распространенные проблемы при работе с benadis-runner.
...
benadis-runner service-mode-disable --infobase MyInfobase
...
du -sh /tmp/benadis-runner/
```

**Решение**:
```bash
# Очистите временные файлы
rm -rf /tmp/benadis-runner/*
...
grep -i password /var/log/benadis-runner.log
```
...
benadis-runner --dry-run service-mode-enable --infobase MyInfobase
```
...
benadis-runner --dump-config

# Показать только определенную секцию
benadis-runner --dump-config --section=app
```
...
strace -e trace=network,file benadis-runner service-mode-enable --infobase MyInfobase

# Профилирование производительности
time benadis-runner service-mode-enable --infobase MyInfobase
```
...
1. **Версию benadis-runner**: `benadis-runner --version`
